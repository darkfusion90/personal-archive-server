...
import {
  getUserAuthStatus,
  performMultifactorLogin,
  performSimpleLogin,
} from "./utils";
import {
  isCurrentDeviceTrusted,
  getUserDeviceInfo,
} from "./multi-factor-auth/utils";
  ...
  passport.authenticate("local", async (err, user, info) => {
    ...
    if (multifactorAuthEnabled) {
      if (await isCurrentDeviceTrusted(req)) {
        ...
        console.log(
          "MultifactorAuth: Current device not trusted. Will respond login failed"
        );
    ...
    console.log(
      "Authentication process completed. Will respond with: ",
      authenticatedUser
    );
